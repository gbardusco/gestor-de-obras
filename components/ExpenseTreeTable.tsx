...
import {
  ...
  Calendar, Truck, CheckCircle2, Circle, GripVertical, Clock, Landmark, Tag, FileText, Download, Receipt
} from 'lucide-react';
    ...
    <div className={`overflow-x-auto border rounded-3xl bg-white dark:bg-slate-900 shadow-xl transition-colors ${isRevenueTable ? 'border-emerald-100 dark:border-emerald-900/40' : 'border-slate-200 dark:border-slate-800'}`}>
      <table className="min-w-full border-collapse text-[11px]">
        ...
        <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
          {data.map((item) => (
            <tr key={item.id} className={`group transition-colors ${item.itemType === 'category' ? 'bg-slate-50/80 dark:bg-slate-800/40 font-bold' : 'hover:bg-slate-50 dark:hover:bg-slate-800/20'}`}>
        ...
        <tfoot className="bg-slate-900 dark:bg-black text-white font-black text-xs sticky bottom-0 z-20 shadow-2xl">
          <tr className="border-t border-white/20">
            <td colSpan={9} className="p-4 text-right uppercase tracking-[0.2em] text-[9px] border-r border-white/10 opacity-70">
