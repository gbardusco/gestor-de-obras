      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="bg-white dark:bg-slate-900 p-8 rounded-[3rem] border border-slate-200 dark:border-slate-800 shadow-xl space-y-8">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Personalização total do PDF</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-4">
                <label className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">
                  <Layout size={12} /> Primárias & Bordas
                </label>
                <ColorInput label="Títulos PDF" value={theme.primary} onChange={(v) => handleChange('primary', v)} />
                <ColorInput label="Destaque Medição" value={theme.accent} onChange={(v) => handleChange('accent', v)} />
                <ColorInput label="Texto do Realce" value={theme.accentText} onChange={(v) => handleChange('accentText', v)} />
                <ColorInput label="Cor das Bordas" value={theme.border} onChange={(v) => handleChange('border', v)} />
              </div>

              <div className="space-y-4">
                <label className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">
                  <Table size={12} /> Estilo de Tabelas
                </label>
                ...
                <ColorInput label="Categoria (Fundo)" value={theme.category.bg} onChange={(v) => handleChange('category.bg', v)} />
                <ColorInput label="Totais (Fundo)" value={theme.footer.bg} onChange={(v) => handleChange('footer.bg', v)} />
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-7 bg-slate-100 dark:bg-slate-950 p-8 sm:p-12 rounded-[3.5rem] border border-dashed border-slate-300 dark:border-slate-800 flex flex-col gap-6 sticky top-8">
          <div className="flex items-center justify-between">
            <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Simulação de Impressão (Preview)</h4>
            <div className="px-3 py-1 bg-white dark:bg-slate-800 rounded-full text-[8px] font-black text-slate-400 uppercase border border-slate-200 dark:border-slate-700">A4 Landscape</div>
          </div>
          ...
          >
            ...
            <div className="flex justify-between items-start border-b-2 pb-4 mb-2" style={{ borderColor: theme.primary }}>
              <div>
                ...
                <h5 className="text-sm font-black uppercase tracking-tight" style={{ color: theme.primary }}>Planilha de Medição de Obras</h5>
                <p className="text-[8px] text-slate-500 font-bold uppercase tracking-widest">Obra Padrão ProMeasure</p>
              </div>
              <div className="text-right">
                <span className="text-[8px] font-black px-2 py-0.5 rounded text-white uppercase tracking-widest" style={{ backgroundColor: theme.accent, color: theme.accentText }}>MEDIÇÃO Nº 01</span>
              </div>
            </div>

            {/* Mock da Tabela do PDF */}
            <div className="border rounded overflow-hidden" style={{ borderColor: theme.border }}>
              <div className="grid grid-cols-4 text-[8px] font-black uppercase tracking-widest p-2" style={{ backgroundColor: theme.header.bg, color: theme.header.text }}>
                ...
                <span className="text-right">Total Período</span>
              </div>
              <div className="grid grid-cols-4 text-[9px] font-bold p-2 border-b" style={{ backgroundColor: theme.category.bg, borderColor: theme.border, color: theme.category.text }}>
                ...
                <span className="text-right">R$ 15.000,00</span>
              </div>
              <div className="grid grid-cols-4 text-[8px] p-2 bg-white border-b" style={{ borderColor: theme.border, color: '#64748b' }}>
                <span className="col-span-2 pl-3">1.1 Escavação Mecanizada</span>
                <span className="text-center">m³</span>
                <span className="text-right font-black" style={{ color: theme.accent }}>R$ 4.250,00</span>
              </div>
